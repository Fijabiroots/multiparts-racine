...
let PdfService = PdfService_1 = class PdfService {
    ...
    extractPurchaseRequisitionItems(text) {
        ...
        const compactPattern = new RegExp(`(\\d{1,4})(${UNITS})(\\d{5,8})([A-Z][A-Z0-9\\s\\-\\.\\/\\&\\,\\(\\)\\:]+?)` +
            `(?:\\s*1500\\d+|\\s+\\d+\\s+\\d+\\s*(?:USD|EUR|XOF)?|\\s*$)`, 'gi');
        let match;
        ...
        if (foundItems.size === 0) {
            const spacedPattern = new RegExp(`\\b(\\d{1,3})\\s+(\\d+)\\s+(${UNITS})\\s+(\\d{5,8})\\s+([A-Z][A-Z0-9\\s\\-\\.\\/\\&\\,\\(\\)\\:]+?)` +
                `(?:\\s+1500\\d+|\\s+\\d+\\s+\\d+\\s*(?:USD|EUR|XOF)?|\\s*$)`, 'gi');
