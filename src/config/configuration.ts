export default () => ({
  app: {
    ...
    // Options pour OCR et révision manuelle
    pdfStoragePath: process.env.PDF_STORAGE_PATH || './storage/pdfs',
    requireManualReviewForOcr: process.env.REQUIRE_MANUAL_REVIEW_FOR_OCR !== 'false', // true par défaut
  },
  imap: {
  ...
  email: {
    ...
    // Accusé de réception
    sendAcknowledgment: process.env.SEND_ACKNOWLEDGMENT !== 'false', // true par défaut
    acknowledgmentDelay: parseInt(process.env.ACKNOWLEDGMENT_DELAY_SECONDS || '5', 10), // Délai avant envoi (secondes)
  },
  reminder: {
    // Relances automatiques
    enabled: process.env.REMINDER_ENABLED !== 'false', // true par défaut
    maxReminders: parseInt(process.env.REMINDER_MAX_COUNT || '3', 10), // Nombre max de relances
    daysBetweenReminders: parseInt(process.env.REMINDER_DAYS_BETWEEN || '2', 10), // Jours entre chaque relance
  },
  lifecycle: {
    // Scan des emails
