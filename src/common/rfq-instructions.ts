/**
 * Instructions RFQ pour les demandes de prix aux fournisseurs
 * Support bilingue: Français / English
 */

export type RfqLanguage = 'fr' | 'en' | 'both';

/**
 * Instructions RFQ en français
 */
export const RFQ_INSTRUCTIONS_FR = `
...
/**
 * Instructions RFQ en anglais
 */
export const RFQ_INSTRUCTIONS_EN = `
<div style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333;">
...
/**
 * Obtenir les instructions RFQ selon la langue
 */
export function getRfqInstructions(language: RfqLanguage = 'both'): string {
  switch (language) {
    case 'fr':
      return RFQ_INSTRUCTIONS_FR;
    case 'en':
      return RFQ_INSTRUCTIONS_EN;
    case 'both':
    default:
      return `
${RFQ_INSTRUCTIONS_FR}
...
/**
 * Détection automatique de la langue basée sur le domaine email
 */
export function detectLanguageFromEmail(email: string): RfqLanguage {
  ...
  for (const ext of frenchDomains) {
    if (domain.endsWith(ext)) return 'fr';
  }
  
  for (const ext of englishDomains) {
    if (domain.endsWith(ext)) return 'en';
  }
  
  // Par défaut: les deux langues
  return 'both';
}
 ...
 */
export function detectLanguageFromText(text: string): RfqLanguage {
  ...
  for (const kw of frenchKeywords) {
    if (lowerText.includes(kw)) frScore++;
  }
