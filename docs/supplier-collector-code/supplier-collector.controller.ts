...
export class SupplierCollectorController {
  ...
  /**
   * GET /api/supplier-collector/sync/folders
   * Liste tous les dossiers IMAP disponibles
   */
  @Get('sync/folders')
  async listFolders() {
    const folders = await this.mailSyncService.listAvailableFolders();
    return {
      folders,
  ...
  /**
   * POST /api/supplier-collector/sync/historical
   * Déclenche une synchronisation historique depuis une date donnée
   * @query since - Date de début (défaut: 2024-01-01)
   * @query folders - Dossiers à synchroniser (optionnel, défaut: auto-détecté)
   * @query batchSize - Nombre max d'emails par dossier (défaut: 0 = pas de limite)
   */
  @Post('sync/historical')
  async triggerHistoricalSync(
    @Query('since') sinceStr?: string,
    @Query('folders') foldersStr?: string,
    @Query('batchSize') batchSizeStr?: string,
  ) {
    const since = sinceStr ? new Date(sinceStr) : new Date('2024-01-01');
    const folders = foldersStr ? foldersStr.split(',').map(f => f.trim()) : undefined;
    // 0 = no limit (fetch ALL emails)
    const batchSize = batchSizeStr ? parseInt(batchSizeStr, 10) : 0;

    this.logger.log(`Historical sync triggered: since=${since.toISOString()}, batchSize=${batchSize}`);

    const result = await this.mailSyncService.historicalSync({
      since,
      folders,
      batchSize,
    });
  ...
  // ============ POSITIVE EMAILS COLLECTION ============

  /**
   * POST /api/supplier-collector/collect-positive-emails
   * Collecte toutes les adresses email des fournisseurs ayant répondu positivement
   * Scanne TOUS les dossiers IMAP depuis janvier 2024 (sans filtre)
   *
   * @query since - Date de début (défaut: 2024-01-01)
   * @query minScore - Score minimum pour être considéré positif (défaut: 3)
   */
  @Post('collect-positive-emails')
  async collectPositiveEmails(
    @Query('since') sinceStr?: string,
    @Query('minScore') minScoreStr?: string,
  ) {
    const since = sinceStr ? new Date(sinceStr) : new Date('2024-01-01');
    const minScore = minScoreStr ? parseInt(minScoreStr, 10) : 3;

    this.logger.log(`Collecting positive emails since ${since.toISOString()}, minScore=${minScore}`);

    const result = await this.mailSyncService.collectPositiveSupplierEmails({
      since,
      minScore,
    });
   ...
   * Récupère les emails positifs déjà en base (sans sync)
   *
   * @query since - Date de début (défaut: 2024-01-01)
   * @query minScore - Score minimum pour être considéré positif (défaut: 3)
   */
  ...
  getPositiveEmails(
    ...
    const result = this.mailSyncService.getPositiveEmailsFromDb({
      since,
      minScore,
    });
